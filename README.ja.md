...
- キーワードやコードでカタログを検索 — `list_stats`（`searchWord` / `statsCode` など）。
- ダウンロード前に表の軸を確認 — `get_meta_info`。
- 必要な部分だけ取得 — `select` が巨大な表を品目・地域・期間でサーバー側に絞り込む。消費者物価指数（約1,300万行）でも、`get_meta_info` が示すのと同じ軸 ID で指定して、必要な数百行だけが返る。
- 大きな表をページングで取得 — `iter_stats_data_pages` が 1 ページずつ返す。`max_rows` で取得上限を設定（`TooManyRowsError`）でき、長い取得は `progress`
  コールバックで追える。

**コードを解読するのではなく、そのまま使えるデータを受け取る**

*既定の動作 — `rule="auto"` でデータを構造化。*

- `000` でなく `男女計` を読む — どのコードも `CLASS_INF` で定義されたラベルを自動的に設定。
- 時間でそのまま並べ替え・集約できる — 日付は正規化され、粒度（`2020`、yearly / monthly / …）を判定。
- 複数行に分散されたデータを畳み込み（pivot）できる — e-Stat が数量、金額のように複数行に分かれていても、ひとつの行にまとめられて、
  測定量ごとに 1 列で横並びに読める（GDP / CPI / 建築着工 / 貿易）。pyestat がまだ構造化できない
  表は、エラーにせず生のデータのまま返る。
- 合計に使える数字だけ残せる — `aggregates="exclude"` で小計・総計行を落とし、`"only"` で
  集計行だけを残すことも可能。
